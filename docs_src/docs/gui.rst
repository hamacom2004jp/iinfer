...
1. 最初に実行するコマンドを選択します。
2. ここでは、 **start** コマンドを選択しました。
3. 続けて **[ + ]** ボタンを押すと、次に実行するコマンドを選択できます。

.. image:: ../static/ss/00311_pipe.jpg

1. startから推論して後処理してstopするパイプラインを設定しました。
2. **Execute** で実行してみます。

.. image:: ../static/ss/00312_pipe.jpg

1. パイプラインで設定したコマンドを順に実行した結果が表示されます。
2. guiモードでのパイプライン設定は、各コマンドのファイル入出力オプションを使用して、パイプラインを流れるデータを設定することになります。
3. コマンドラインモードでは、標準入出力をパイプ「|」でつないでいくことで、パイプライン構成することが出来ます。
