    ...
    def get_option(self):
        ...
        return dict(
            ...
            choice=[
                ...
                dict(opt="svname", type=Options.T_STR, default=self.default_svname, required=False, multi=False, hide=True, choice=None,
                     description_ja="推論サーバーのサービス名を指定します。省略時は `server` を使用します。",
                     description_en="Specify the service name of the inference server. If omitted, `server` is used."),
                dict(opt="maxrecsize", type=Options.T_INT, default=1000, required=False, multi=False, hide=True, choice=None,
                     description_ja="Redisサーバーに保存する推論結果の最大レコードサイズを指定します。",
                     description_en="Specifies the maximum record size of inference results to be stored on the Redis server."),
