...
class InstallMmpretrain(iinfer_install_mmcv.InstallMmcv):
    ...
    def get_option(self):
        ...
        opt['description_ja'] = "`mmpretrain` をインストールします。"
        opt['description_en'] = "Install `mmpretrain`."
        opt['choice'] += [
    ...
    def apprun(self, logger:logging.Logger, args:argparse.Namespace, tm:float, pf:List[Dict[str, float]]=[]) -> Tuple[int, Dict[str, Any], Any]:
        ...
        ret = self.mmpretrain(logger, Path(args.data), install_use_gpu=args.install_use_gpu)
    ...
    def mmpretrain(self, logger:logging.Logger, data_dir:Path, install_use_gpu:bool=False):
        returncode, _, _cmd = common.cmd(f'git clone https://github.com/open-mmlab/mmpretrain.git', logger=logger, slise=-1)
        if returncode != 0:
            logger.warning(f"Failed to git clone mmpretrain. Delete mmpretrain as it probably already exists. cmd:{_cmd}")
        ...
        if ret != 0:
            logger.warning(f"Failed to install mmpretrain. cmd:{_cmd}")
