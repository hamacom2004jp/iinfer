        ...
        return dict(
            ...
            choice=[
                ...
                dict(opt="json_without_img", type=Options.T_BOOL, default=False, required=False, multi=False, hide=True, choice=[True, False],
                     description_ja="JSONの送信時に画像を含めず送信します。",
                     description_en="Send JSON without including images when sending JSON."),
                dict(opt="fileup_name", type=Options.T_STR, default="file", required=True, multi=False, hide=False, choice=None,
                     description_ja="推論結果の画像をPOSTするときのパラメータ名を指定します。省略すると `file` が使用されます。",
                     description_en="Specify the parameter name when posting the image of the inference result. If omitted, `file` is used."),
                dict(opt="outputs_url", type=Options.T_STR, default=None, required=True, multi=False, hide=False, choice=None,
                     description_ja="推論結果のJSONをPOSTするURLを指定します。",
                     description_en="Specify the URL to POST the JSON of the inference result."),
                dict(opt="output_image_url", type=Options.T_STR, default=None, required=False, multi=False, hide=False, choice=None,
                     description_ja="推論結果の画像をPOSTするURLを指定します。",
                     description_en="Specify the URL to POST the image of the inference result."),
                dict(opt="output_image_ext", type=Options.T_STR, default='jpeg', required=False, multi=False, hide=False, choice=['bmp', 'png', 'jpeg'],
                     description_ja="推論結果の画像をフォーマットを指定します。 `bmp` , `png` , `jpeg` が指定できます。",
                     description_en="Specifies the format of the image of the inference result.You can specify `bmp` , `png`, or `jpeg`."),
                dict(opt="output_image_prefix", type=Options.T_STR, default='output_', required=False, multi=False, hide=False, choice=None,
                     description_ja="推論結果の画像の接頭語を指定します。省略すると `output_` が使用されます。",
                     description_en="Specifies the prefix of the inferred result image. If omitted, `output_` is used."),
