...
class PostprocessCmd(postprocess_feature.PostprocessFeature):
    ...
    def get_option(self):
        ...
        return dict(
            ...
            description_ja="推論結果を環境変数にセットし任意のコマンドを実行します。",
            description_en="Set the inference result to an environment variable and execute an arbitrary command.",
            choice=[
                ...
                dict(opt="cmdline", type=Options.T_STR, default='cwd', required=True, multi=False, hide=False, choice=None,
                     description_ja="実行するコマンドを指定します。設定される環境変数は `outputs` , `output_image` です。この値は一時ファイルのファイルパスです。",
                     description_en="Specifies the command to execute. The environment variables set are `outputs` , `output_image`. The value is the file path of the temporary file."),
                dict(opt="output_image_ext", type=Options.T_STR, default="jpeg", required=True, multi=False, hide=True, choice=['bmp', 'png', 'jpeg'],
                     description_ja="出力画像のフォーマットを指定します。指定可能な画像タイプは `bmp` , `png` , `jpeg`",
                     description_en="Specifies the format of the output image.Acceptable image types are `bmp` , `png`, and `jpeg`."),
                dict(opt="output_maxsize", type=Options.T_INT, default=1024*1024*5, required=True, multi=False, hide=True, choice=None,
                     description_ja="コマンド実行結果をキャプチャーする最大サイズを指定します。",
                     description_en="Specifies the maximum size of the command execution results to be captured."),
