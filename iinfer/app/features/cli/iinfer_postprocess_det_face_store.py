...
class PostprocessDetFaceStore(postprocess_feature.PostprocessFeature):
    ...
    def get_option(self):
        ...
        return dict(
            ...
            description_ja="Face Detection and Recognitionで検知した顔特徴データを個所を切り出し、顔認識ストアファイルを生成します。",
            description_en="Cut out the face feature data detected by Face Detection and Recognition and generate a face recognition store file.",
            choice=[
                ...
                dict(opt="face_threshold", type=Options.T_FLOAT, default=0.0, required=False, multi=False, hide=False, choice=None,
                     description_ja="顔スコアが閾値以下の場合は、顔特徴量ストアに含まれないようにします。",
                     description_en="If the face score is below the threshold, it will not be included in the face feature store."),
