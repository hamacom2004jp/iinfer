...
class OnnxdetYoloXLite(onnx_det_YoloX.OnnxDetYoloX):
    ...
    def predict(session, img_width:int, img_height:int, image:Image, labels:List[str]=None, colors:List[Tuple[int]]=None):
        """
        予測を行う関数です。
        ...
        """
        return super().predict(session, img_width, img_height, image, labels=labels, colors=colors)
